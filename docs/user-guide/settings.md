...
Immediately before the catalog, the system message explains the closed first-party intent vocabulary: `test-`, `audit-`, `trace-`, `analyze-`, `operate-`, `assess-`, and `plan-`. The prefix helps the model distinguish testing, inspection, tracing, offline analysis, tool operation, broad assessment, and planning, but explicitly grants no authorization, tool access, or execution right. Cyberful rejects a first-party package outside this lowercase kebab-case vocabulary; explicitly trusted extension names remain compatible without being rewritten.
...
`skill_search`, `skill_read`, and `skill_stage` are eagerly available to every root, child, and fallback run. Use search when the relevant name is ambiguous; exact and prefix names outrank category, trigger, tag, framework identifier, and description matches. Search always uses the complete in-memory index, even when an entry is name-only in the prompt, and returns bounded metadata without loading instructions or exposing host paths. Once a name is selected, `skill_read` must load the complete `SKILL.md`; only that complete instruction read marks the skill as used. When the procedure needs a packaged script or asset, the same AgentRun calls `skill_stage` after the instruction read. Staging copies one regular file below `scripts/` or `assets/` byte-for-byte into `raw/skill-resources/<skill>/<package-digest>/`, rejects traversal, symlinks, special files and oversized resources, and returns only its workarea-relative path, byte count and SHA-256 digest. References and agents remain package-bound and are read only as required. `allow_project_discovery` must remain `false`; additional trusted content is admitted only through the two explicit root lists.
