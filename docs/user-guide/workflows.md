...
Every Pentest, Bug Bounty, Code Audit, and Ask phase can query the release-embedded MITRE ATT&CK snapshot without network access. ATT&CK is the authoritative source for its own identifiers, definitions, and relationships, but only a non-exhaustive reasoning lens: the workflows continue to seek zero-days, new primitives, business-logic failures, application-specific weaknesses, and chains outside its matrices. A missing or rejected mapping never weakens a supported technical finding. See [MITRE ATT&CK MCP](../runtimes/mitre-attack.md).

Every Pentest and Bug Bounty phase, plus Ask, can use `web_search` for unauthenticated public discovery. The gateway always routes it to the phase-ephemeral, direct `search` browser identity; `agent_browser_*` calls may select the same identity only for DuckDuckGo or Google search pages because the host fixes its native allowlist to those two domain families. Result and target hosts require an authorized numbered ZAP-routed profile, whose account state remains isolated in profiles 1–5. Search results never define or expand Brief scope, never enter target surface coverage or Recon handoff summaries, and never replace engagement evidence in Report. Code Audit publishes no browser or web-search capability and remains offline.
...
After that shutdown, every accepted Pentest and Bug Bounty phase receives a best-effort host-owned ZAP passive-evidence checkpoint before its successor starts. Only observed HTTP(S) origins whose hostname matches the finalized `authorized_http_hosts` policy are included. The host does not create findings or hypotheses from scanner alerts, waits for the passive queue for at most 10 seconds, and never blocks the accepted handoff when collection is partial or failed. A non-web engagement finalizes `authorized_http_hosts: []`, writes the silent `not_applicable` checkpoint, and makes no ZAP call or wait; Code Audit and Ask never run the checkpoint. Current manifests live at `raw/zap/passive/<workflow>/<phase>.json`, immutable report objects under `raw/zap/passive/objects/`, and the append-only record at `raw/operations/zap-passive-evidence.jsonl`. Report consumes the Verify checkpoint; the post-Report checkpoint is archival.

An active-execution budget applies to active work. The final three to five minutes are a host-enforced closeout: the same root stops researching, children are cancelled, research tools are blocked, and only local evidence, deliverable/ledger reconciliation, cleanup, and handoff remain. Before that reserve, the first valid `exhausted` synthesis in web Recon receives one positive breadth continuation; Exploit and Hacker receive it when a ready profile is unused or predecessor ZAP route families remain coverage candidates. Active subagents are not cancelled by early exhaustion: the root is told to wait for and integrate them. A second exhausted synthesis may close normally, candidates need not be reduced to zero, and there is no click quota or requirement to explain a phase with no browser activity. The event stream records the decision as `phase_research_continuation`. If the final deadline expires, Cyberful advances a research phase in degraded mode only when the partial artifact exists, can be sealed, and every phase-owned process has stopped. Brief never advances from a partial `MISSION.md` without an explicit handoff. Invalid handoffs, missing artifacts, failed integrity gates, and incomplete cleanup halt the chain.
...
Each hypothesis in `raw/hypotheses/registry.json` has one state: `OPEN`, `TESTING`, `QUEUED`, `CONFIRMED`, `DISPROVED`, `SUSPECTED`, `INCONCLUSIVE`, or `UNTESTABLE`. New entries declare the primary observation, positive, negative, and invalid conditions, and controls before testing. They begin with an ATT&CK applicability assessment of `UNASSESSED`; agents may record a rationale, evidence references, and supported mappings or explicitly choose `NOT_APPLICABLE` or `UNAVAILABLE`. The host attaches the release snapshot identity, while agents remain responsible for the case-specific mapping. The oracle is immutable once testing begins; an older entry without one remains readable and may receive it through its next `claim`. `UNTESTABLE` means the discriminating test never ran and therefore records a typed blocker plus an exact next step. This prevents missing access, tools, applicability, authority, or budget from inflating the suspected-finding count.

Call `hypothesis claim` before a first discriminator or retest. Claim is atomic and refuses a competing live owner; the legacy `hypothesis update` with `state: TESTING` uses the same implementation. Closing a test records `test_result`: `POSITIVE` is required for `SUSPECTED`, `CONFIRMED`, and `promote`; `NEGATIVE` is required for `DISPROVED`; `INVALID` or an explicitly described `CONFLICT` is required for `INCONCLUSIVE`. Its primary and derived evidence paths must be safe workarea-relative paths. Inspect primary artifacts first: raw target, process, or program observations remain authoritative evidence, while parser, scanner, and classifier interpretations are derived and any disagreement must remain visible. Executed dispositions are rejected unless the current state is `TESTING`; `OPEN` may still move directly to `QUEUED` or `UNTESTABLE`, and a queued hypothesis enters `TESTING` through `reopen` in its named successor. After positive evidence, `hypothesis promote` durably creates or reuses the finding and links both registries in one retry-safe operation.

Promotion copies the complete ATT&CK assessment into the finding without treating it as proof. Only Verify can assign the final `ACCEPTED`, `REVISED`, or `REJECTED` mapping review; Report publishes only accepted or revised mappings. Neither registry validates case-specific ATT&CK IDs at runtime or requires evidence of a previous MCP call, and absent mappings do not block handoff.

`CONFIRMED` and `SUSPECTED` entries must link current-run findings. Negative-only outcomes can retain stable backlog IDs when they never met the positive-evidence threshold for entering the finding registry. Closed and queued hypotheses keep their stable IDs, evidence, and transition history across phases; each retest adds its result to the closing transition rather than overwriting an earlier outcome, and Report reads the complete registry rather than a reconstructed subset. The TUI hypothesis detail shows the oracle, latest result, evidence provenance, conflicts, and per-transition result summaries without introducing another ledger or screen.
...
When the authorized target contains a reachable web application, Recon begins with breadth across real product journeys and state transitions before concentrating on its strongest hypotheses. Exploit continues navigating functionality, alternative workflows, roles, lifecycle, and unvisited states while testing candidates. Hacker actively uses product areas not yet connected when seeking chains. Bug Bounty treats this breadth as reward discovery. Route variants are coverage rather than causal novelty, but that distinction never justifies stopping exploration while authorized functionality remains accessible.

The version-3 coverage ledger at `raw/operations/surface-coverage.jsonl` deliberately separates three sources. `http_surface` is the canonical passive HTTP map from ZAP history; it contains only ZAP ID, in-scope origin, redacted route family, method, status, and response presence. `browser_activity` contains only target profile, opaque tab, semantic family, outcome, and reliable current origin. `direct_egress` retains cyberful-os observations and protocols that ZAP cannot represent. Queries, headers, bodies, cookies, selectors, inserted values, and activity from the `search` profile are excluded. Passive out-of-scope dependencies remain visible in a separate summary section and do not drive handoff or continuation.

The gateway synchronizes ZAP history in pages of at most 100 after target-profile agent-browser operations, after ZAP request or replay operations, and before exhausted synthesis, handoff, and gateway shutdown. An oversized response halves the page size without advancing the durable cursor; the safe size is retained for that synchronization. The cursor at `raw/operations/zap-history-coverage.cursor.json` resumes across phase change and recovery and advances only after durable coverage; a missing cursor safely replays history because HTTP summaries are set-based. This collector neither generates requests nor waits for passive scanning. A collection error records a typed degraded cause without changing a successful browser action into an error, while a true required-ZAP transport failure retains the normal upstream failure semantics.

Per-phase summaries under `raw/operations/surface-coverage/` expose HTTP routes, methods, statuses, passive dependencies, UI action-family counts, per-profile counts, and collector state. Exploit compares its current ZAP routes with Recon; Hacker compares with Recon and Exploit. Missing predecessor routes are suggestions named `coverage_candidates`, never obligations to replay. Recon alone retains a handoff guard: every profile that the current policy marks `READY + IN_SCOPE` must complete one successful navigation, interaction, input, or script action on its configured origin during Recon itself. Brief readiness and observation-only calls no longer satisfy that requirement.
