...
After submission, Cyberful moves the prompt into the session view. The session feed shows user messages, assistant output, tool activity, and workflow status. The composer's fixed shoulder reads **Ready** while idle. During work it shows **Working**, the active phase's elapsed time and name, and whether Pi is generating or executing a job; generic host work uses its existing bounded status message. This is observed activity rather than a completion percentage. The composer remains available for the next prompt or for steering an active turn, while dialogs and full-screen feature views use higher overlay layers than prompt autocomplete.
...
The hypothesis block begins with a muted `(i) N active hypotheses` row when the workflow has unresolved `OPEN`, `QUEUED`, `TESTING`, or `SUSPECTED` hypotheses. Each active hypothesis then appears as a muted, focusable preview with its stable ID, description, state, and phase. Findings always remain above hypotheses, and a light divider appears before the hypothesis count and item list. When there are no findings, the empty finding block shows a muted `0` with one blank row above and below before that divider. Click a hypothesis, or focus it and press `Enter`, to open its read-only detail with ownership, root cause, surface, discriminator, tool choices, blocker or next step, evidence, linked references, and transition history. `CONFIRMED`, `DISPROVED`, `INCONCLUSIVE`, and `UNTESTABLE` are not counted or listed. The host hydrates this view on session open and refreshes by registry revision, so restarts and child-ownership recovery remain accurate.
