...
`runtime_status` reports and actively checks only the managed browser and Ghidra MCP generations. Call it with `action=status` to discover the labels active in the current phase, then pass one returned label such as `browser-1` or `browser-search` to `action=check`. A label that is not active or not managed is rejected as non-retryable and points back to status discovery; it is never reported as a transport health failure.
