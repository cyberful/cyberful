...
| `skill_search` | Cyberful release | Eager metadata-only progressive skill search. | Find any of the 106 built-in skills by exact or prefix name, category, capability, tag, trigger, or identifier from the eight pinned framework namespaces, including PCI DSS and GDPR, before reading its complete instructions. |
...
| `finding` | Cyberful release | Authoritative session finding and maturation registry. | Record, update, verify, classify, mature, alias, list, and read evidence-backed security findings across phases; Bug Bounty Verify separates reproduced mechanisms from submission-ready invariant violations and unwanted attacker effects. |
...
| `hypothesis` | Gateway 0.1.0 | Persistent vulnerability hypothesis registry with immutable test oracles, primary-versus-derived evidence provenance, transition-scoped results, atomic ownership claims, ordered idempotent positive `promote`, Bug Bounty reward context, append-only context correction, and structured portfolio synthesis with qualitative opportunity closeout. | Declare a discriminating oracle, claim each test or retest, close it with a structurally consistent positive, negative, invalid, or conflicting result, preserve raw observations independently from classifier output, promote positive findings, converge, pivot, exhaust, and hand off evidence-backed research without duplicate actors or silent evidence overrides. |
...
The browser MCP is version 0.1.0 and uses Patchright 1.61.1. The gateway owns one lazy Chromium hub for each profile; AgentRuns attach isolated controllers whose tabs, active page, DOM refs, and observed traffic remain private. Target profiles 1–5 retain proxy attestation, while the named `search` profile is direct.
...
| `browser_status` | MCP 0.1.0; Patchright 1.61.1 | Attest the configured proxy, then report shared profile state and only this AgentRun's tabs. | Inspect the shared profile runtime plus the caller's active tab, URLs, titles, and owned-tab list without exposing sibling controllers. |
| `browser_tabs` | MCP 0.1.0; Patchright 1.61.1 | List, open, select, or close tabs owned by this AgentRun. Foreign and stale tab ids are reported as nonexistent. | Manage multiple private tabs within one AgentRun while the selected Chromium profile remains shared. |
...
| `browser_captcha_handoff` | MCP 0.1.0; Patchright 1.61.1 | Attest an already-visible CAPTCHA/challenge and bring the browser to the front. Then ask the human with the gateway question tool using kind=captcha; only this AgentRun, profile, tab, and origin pause until browser_captcha_status clears the original page. | Pause the exact challenged tab for human completion without blocking sibling tabs or AgentRuns. |
...
| `browser_network_log` | MCP 0.1.0; Patchright 1.61.1 | Return recent captured network requests and responses from this AgentRun's owned tabs. | Inspect traffic observed by the caller's tabs without mixing requests from sibling controllers. |
| `browser_network_response_body` | MCP 0.1.0; Patchright 1.61.1 | Read a controller-local response body by network request id from browser_network_log. | Read a stored body only from the caller's private network namespace. |
...
| `browser_close` | MCP 0.1.0; Patchright 1.61.1 | Close the selected shared browser profile. The gateway exposes this only to the original phase root; other AgentRuns close owned tabs with browser_tabs. | Let only the phase root terminate every controller and the Chromium hub for one selected profile. |
