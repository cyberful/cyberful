...
| `hypothesis` | Gateway 0.1.0 | Persistent vulnerability hypothesis registry with atomic ownership claims, ordered idempotent `promote`, Bug Bounty reward context, append-only context correction, and structured portfolio synthesis with qualitative opportunity closeout. | Create, contextualize, claim, test, promote, disprove, converge, pivot, exhaust, account for untested authorized reward opportunities, and hand off evidence-backed research hypotheses without duplicate actors, invented reward mappings, or prose-only novelty claims. |
