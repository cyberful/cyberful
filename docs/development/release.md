...
Each release build separately resolves the current Enterprise, Mobile, and ICS versions from the official MITRE ATT&CK STIX index exactly once. The snapshot preparation job validates and indexes those versioned bundles, records every digest in a manifest, and distributes the same prepared directory to all native build jobs. Each job re-verifies it before embedding, and assembly rejects any binary whose manifest differs from the prepared snapshot.

Engagement runtime never contacts MITRE, GitHub, or TAXII. The release also publishes `cyberful-mitre-attack-<snapshot>.tar.gz`, containing the original STIX bundles, derived SQLite/FTS database, manifest, checksums, SPDX metadata, and the exact ATT&CK license. The version is release-specific and must be read from the manifest rather than hardcoded in source or documentation.

A dry run verifies source, tests, documentation, native binaries, the embedded runtime manifest, the shared ATT&CK snapshot and offline MCP smoke test, npm archives, SPDX metadata, and checksums. It publishes nothing. A live run then creates or verifies the annotated source tag and draft GitHub Release, uploads immutable assets, publishes the platform packages followed by the launcher through npm OIDC, and makes the GitHub Release public only after npm succeeds.
