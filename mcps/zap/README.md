...
These sources are copied into the unified cyberful-os image. Cyberful runs one dedicated, engagement-scoped ZAP container from that image and one short-lived `docker exec` bridge process per eligible phase gateway. The ZAP service and bridge communicate on container loopback; only ZAP port 8080 may be published on host loopback.
...
`zap_history_search` and `zap_history_get` return metadata by default. Search reads split the requested raw-history range into upstream chunks of at most 100 messages, halve an oversized chunk until it fits the bridge response ceiling, and project each chunk before continuing. `returned` counts matching results, while `scanned` and `next_start` describe the raw-history range consumed so filtered pagination cannot skip or repeat records. Complete headers and bodies require `include_bodies: true` and remain limited to one adaptively reduced page. Large or binary results are content-addressed under `raw/zap/history/objects/`, so repeated pages and message reads reuse one on-disk value instead of emitting timestamp-named duplicates. Read those returned workarea-relative paths with `workarea_read`; they are evidence artifacts, not imported source. The generic `core/view/message` and `core/view/messages` operations remain available when an agent needs the native ZAP response.
