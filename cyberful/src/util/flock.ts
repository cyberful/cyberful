// ── Cooperative Filesystem Lock ──────────────────────────────────
// Serializes promise-based state mutations with owner tokens, stale-lock
// recovery, bounded retry, and heartbeat updates.
// → cyberful/src/util/effect-flock.ts — provides the scoped Effect variant.
// ─────────────────────────────────────────────────────────────────
...
export namespace Flock {
  ...
  function timing(name: string, value: number) {
  ...
  async function tryAcquireLockDir(lockDir: string, opts: Opts): Promise<Owned | { acquired: false }> {
    ...
    await writeFile(heartbeatPath, "", { flag: "wx" }).catch(async (error: unknown) => {
      await rm(lockDir, { recursive: true, force: true })
      if (code(error) === "EEXIST") {
        throw new Error("Lock acquired but heartbeat already existed (possible compromise).", { cause: error })
      }
      throw error
    })

    await writeFile(metaPath, JSON.stringify(meta, null, 2), { flag: "wx" }).catch(async (error: unknown) => {
      await rm(lockDir, { recursive: true, force: true })
      if (code(error) === "EEXIST") {
        throw new Error("Lock acquired but meta.json already existed (possible compromise).", { cause: error })
      }
      throw error
    })
    ...
    const startHeartbeat = (intervalMs = Math.max(100, Math.floor(opts.staleMs / 3))) => {
      if (timer) return
      timing("heartbeat interval", intervalMs)
      timer = setInterval(() => {
        heartbeatTask = heartbeatTask
          .then(() => {
            const timestamp = new Date()
            return utimes(heartbeatPath, timestamp, timestamp)
          })
          .catch((error: unknown) => {
    ...
    const releaseOwned = async () => {
      ...
      if (heartbeatError !== undefined) {
        throw new Error("Refusing to release: lock heartbeat failed and ownership may be compromised.", {
    ...
    const release = () => (releaseTask ??= releaseOwned())
  ...
  export async function acquire(key: string, input: Options = {}): Promise<Lease> {
    input.signal?.throwIfAborted()
    const cfg: Opts = {
      staleMs: timing("staleMs", input.staleMs ?? defaultOpts.staleMs),
      timeoutMs: timing("timeoutMs", input.timeoutMs ?? defaultOpts.timeoutMs),
      baseDelayMs: timing("baseDelayMs", input.baseDelayMs ?? defaultOpts.baseDelayMs),
      maxDelayMs: timing("maxDelayMs", input.maxDelayMs ?? defaultOpts.maxDelayMs),
    }
