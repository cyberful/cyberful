...
// Normalizes remote and local repository identifiers into stable host, owner,
// repository, and display fields.
// → cyberful/src/reference/reference.ts — consumes normalized references.
// ─────────────────────────────────────────────────────────────────
...
function githubRemote(pathname: string) {
  const base = process.env.CYBERFUL_REPO_CLONE_GITHUB_BASE_URL
  if (!base) return `https://github.com/${pathname}.git`
  let url: URL
  try {
    url = new URL(`${pathname}.git`, withSlash(base))
  } catch (cause) {
    throw new Error("CYBERFUL_REPO_CLONE_GITHUB_BASE_URL must be an absolute URL", { cause })
  }
  if (url.protocol !== "http:" && url.protocol !== "https:") {
    throw new Error("CYBERFUL_REPO_CLONE_GITHUB_BASE_URL must use http or https")
  }
  return url.href
}
...
export function validateRepositoryBranch(branch: string) {
  ...
  if (invalid) {
    throw new InvalidRepositoryBranchError({
      branch,
      message: "Branch must be a safe Git branch name using alphanumeric characters, /, _, ., and -",
    })
  }
}
