...
// Resolves local and Git-backed project references, detects cache conflicts, and
// materializes only the repositories required by the active runtime policy.
// → cyberful/src/reference/repository-cache.ts — owns safe cached Git materialization.
// → cyberful/src/config/reference.ts — normalizes configured reference entries.
// ─────────────────────────────────────────────────────────────────
...
export class Service extends Context.Service<Service, Interface>()("@cyberful/Reference") {}
