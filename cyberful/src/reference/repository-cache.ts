// ── Locked Reference Repository Cache ────────────────────────────
// Validates remote references and branches, then clones or refreshes one
// canonical cache path under an inter-process lock.
// → cyberful/src/reference/reference.ts — resolves configured references onto this cache.
// → cyberful/src/util/repository.ts — validates and canonicalizes repository identities.
// ─────────────────────────────────────────────────────────────────
...
export class Service extends Context.Service<Service, Interface>()("@cyberful/RepositoryCache") {}
...
const ensureWithServices = Effect.fn("RepositoryCache.ensureWithServices")(function* (
  ...
  // ── Cache Mutation Has One Inter-Process Owner ──────────────────
  // A canonical repository identity maps to one local path that concurrent
  // Cyberful processes could otherwise clone or reset simultaneously. The lock
  // spans inspection, stale-cache replacement, fetch, checkout, and reset. A
  // directory is reused only when its origin resolves to the requested identity;
  // all destructive operations remain confined to the derived cache path.
  // ─────────────────────────────────────────────────────────────────
  return yield* Effect.acquireUseRelease(
    ...
    (lock) =>
      Effect.tryPromise({ try: () => lock.release(), catch: (cause) => cause }).pipe(
        Effect.catch((error) =>
          Effect.logWarning("failed to release repository cache lock").pipe(
            Effect.annotateLogs({ localPath, error: errorMessage(error) }),
          ),
        ),
      ),
  )
