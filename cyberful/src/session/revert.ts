// ── Session Revert Lifecycle ───────────────────────────────────────────
// Reverts workspace patches to a selected message boundary and removes superseded history.
// → cyberful/src/snapshot/index.ts — tracks, restores, and diffs filesystem state.
// → cyberful/src/session/session.ts — persists revert metadata and message removal.
// ─────────────────────────────────────────────────────────────────────────
...
export class Service extends Context.Service<Service, Interface>()("@cyberful/SessionRevert") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cleanup = Effect.fn("SessionRevert.cleanup")(function* (session: Session.Info) {
      ...
      yield* sessions.withMessageWrite(
        sessionID,
        Effect.gen(function* () {
