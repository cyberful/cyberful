// ── Per-Project Formatting Service ───────────────────────────────
// Resolves configured formatter precedence once per project and applies each
// matching formatter sequentially to a changed file.
// → cyberful/src/format/formatter.ts — detects formatter capabilities and commands.
// → cyberful/src/effect/process.ts — owns subprocess output limits and cancellation.
// ─────────────────────────────────────────────────────────────────
...
export class Service extends Context.Service<Service, Interface>()("@cyberful/Format") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Format.state")(function* (ctx) {
        ...
        function formatFile(filepath: string) {
          return Effect.gen(function* () {
            ...
            for (const { item, cmd } of formatters) {
              ...
              const executable = replaced[0]
              if (!executable) {
                log.error("formatter command is empty", { formatter: item.name, file: filepath })
                continue
              }
              const dir = yield* InstanceState.directory
              const result = yield* appProcess
                .run(
                  ChildProcess.make(executable, replaced.slice(1), {
                    cwd: dir,
        ...
        // ── Ruff And uv Are One Python Formatting Capability ────────
        // The uv formatter is a fallback only when a standalone Ruff command is
        // unavailable; running both would apply the same formatter twice. Config
        // therefore treats either disable flag as disabling the shared capability,
        // while ordinary overrides still merge into their named implementation.
        // This keeps discovery and per-file execution consistent for Python users.
        // ─────────────────────────────────────────────────────────────────
        if (cfg.formatter !== true) {
          for (const [name, item] of Object.entries(cfg.formatter)) {
            const builtIn = Object.entries(Formatter).find(([key]) => key === name)?.[1]
