...
describe("AgentPromptCompiler", () => {
  test("produces one complete system message and keeps the assigned task in one user message", () => {
    ...
    expect(intent).toBeLessThan(catalog)
    for (const prefix of ["test-", "audit-", "trace-", "analyze-", "operate-", "assess-", "plan-"])
      expect(compiled.system).toContain(`- ${prefix}*`)
    expect(compiled.system).toContain("never grant authorization/tools/execution")
