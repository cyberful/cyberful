...
// including structured subsystem failures, host dynamic tools, activity mapping,
// and Codex-specific process configuration.
...
export interface SubsystemRunSpec {
  ...
  // Complete per-phase Cyberful contract replacing Codex's model-specific base instructions.
...
export type SubsystemFailureKind = "security_policy_block" | "transport" | "authentication" | "capacity" | "unknown"
...
export interface AgenticSubsystemAdapter {
  ...
  classifyFailure(completedTurn: unknown): SubsystemFailure | undefined
}

export type Subsystem = AgenticSubsystemAdapter

// ── Actor References Resolve Inside One Subsystem Run ────────────
// Subsystems may learn an actor's readable identity from one lifecycle event
...
function codexFailure(completedTurn: unknown): SubsystemFailure | undefined {
...
export const codex: Subsystem = {
  ...
  streamActivities(event) {
      ...
      return [
        {
          ...
          tool: "codex.settings",
          input: {
            requestedEffort: SubsystemCodex.effort(),
        ...
        {
          kind: "output",
          text: `Requested effort=${SubsystemCodex.effort()}, resolved effort=${settings.effort ?? "null"}, multiAgentMode=${settings.multiAgentMode || "missing"}`,
          callID,
        },
      ]
    }
    ...
    if ((event.method === "item/started" || event.method === "item/completed") && isRecord(event.params)) {
      ...
      if (item.type === "reasoning")
        return [
          {
            kind: "reasoning",
            itemID: callID || `${actor?.id ?? "thread"}:reasoning`,
            hasSummary: reasoningFieldPresent(item.summary),
            hasContent: reasoningFieldPresent(item.content) || reasoningFieldPresent(item.text),
            hasDelta: false,
            ...actorContext,
          },
        ]
    ...
    if (typeof event.method === "string" && /reasoning/i.test(event.method) && /delta/i.test(event.method)) {
      const params = isRecord(event.params) ? event.params : {}
      return [
        {
          kind: "reasoning",
          itemID: typeof params.itemId === "string" ? params.itemId : `${actor?.id ?? "thread"}:reasoning-delta`,
          hasSummary: /summary/i.test(event.method),
          hasContent: !/summary/i.test(event.method),
          hasDelta: true,
          ...actorContext,
        },
      ]
    ...
    if (item.type === "reasoning")
      return [
        {
          kind: "reasoning",
          itemID: callID || "thread:reasoning",
          hasSummary: reasoningFieldPresent(item.summary),
          hasContent: reasoningFieldPresent(item.content) || reasoningFieldPresent(item.text),
          hasDelta: false,
        },
...
function reasoningFieldPresent(value: unknown): boolean {
