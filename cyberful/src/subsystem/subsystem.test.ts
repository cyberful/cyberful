...
// → cyberful/src/subsystem/subsystem.ts — implements the tested adapter.
// ─────────────────────────────────────────────────────────────────
...
const gateway: Subsystem.SubsystemMcpServer = {
  name: "expert-gateway",
  command: "/bun",
  args: ["/gateway.ts"],
  env: {
...
describe("codex adapter", () => {
  test("classifies only a terminal structured cyber policy failure", () => {
    expect(
      Subsystem.codex.classifyFailure({
        status: "failed",
    ...
    expect(Subsystem.codex.classifyFailure({ status: "completed", safetyBuffering: true })).toBeUndefined()
    expect(
      Subsystem.codex.classifyFailure({
        status: "failed",
    ...
    expect(
      Subsystem.codex.classifyFailure({ status: "failed", error: { codexErrorInfo: "unauthorized" } }),
  ...
  test("runs ephemeral JSONL with ignored personal config, workspace sandbox, network and no approvals", () => {
    ...
    expect(args.join(" ")).not.toContain("cyberful base contract")
  ...
  test("readonly selects the read-only sandbox", () => {
    const { args } = Subsystem.codex.buildArgs(at({ kind: "readonly" }))
  ...
  test("builds an isolated app-server transport with one explicit gateway", () => {
    const built = withWebSearch("1", () =>
      Subsystem.codex.buildAppServerArgs(
        at(
          { kind: "autonomous" },
          {
            model: "gpt-5.6-sol",
    ...
    expect(built.args.join(" ")).not.toContain("cyberful base contract")
    expect(built.args.some((arg) => arg.startsWith("developer_instructions="))).toBe(false)
  ...
  test("disables the Codex web search tool when WEB_SEARCH=0", () => {
    const built = withWebSearch("0", () => [
      Subsystem.codex.buildArgs(at({ kind: "autonomous" })),
      Subsystem.codex.buildAppServerArgs(at({ kind: "autonomous" })),
    ])
  ...
  test("enforces an offline sandbox independently of the global web-search setting", () => {
    const built = withWebSearch("1", () => [
      Subsystem.codex.buildArgs(at({ kind: "autonomous" }, { networkAccess: false })),
      Subsystem.codex.buildAppServerArgs(at({ kind: "autonomous" }, { networkAccess: false })),
    ])
  ...
  test("extracts the last completed agent message", () => {
    ...
    expect(Subsystem.codex.extractResultText(stdout)).toBe("final")
  })
  ...
  test("extracts and maps app-server notifications", () => {
    ...
    expect(Subsystem.codex.extractResultText(JSON.stringify(completed))).toBe("app-server final")
    expect(Subsystem.codex.streamActivities(completed)).toEqual([{ kind: "text", text: "app-server final" }])
    expect(
      Subsystem.codex.streamActivities({
        method: "item/started",
  ...
  test("maps the app-server thread total to subsystem-neutral generated-token usage", () => {
    expect(
      Subsystem.codex.streamActivities({
  ...
  test("surfaces resolved Codex settings as one paired diagnostic card", () => {
    expect(
    ...
    ).toEqual([
      {
        kind: "tool",
        tool: "codex.settings",
        input: { effort: "ultra", requestedEffort: "ultra", multiAgentMode: "explicitRequestOnly" },
        callID: "codex-settings-thr_1",
      },
      {
        kind: "output",
        text: "Requested effort=ultra, resolved effort=ultra, multiAgentMode=explicitRequestOnly",
        callID: "codex-settings-thr_1",
      },
    ])
  })
  ...
  test("maps native spawn and child activity into collaboration and lifecycle rows", () => {
    ...
    expect(Subsystem.codex.streamActivities({ method: "item/started", params: { item: spawn } })).toEqual([
      {
        kind: "tool",
    ...
    expect(Subsystem.codex.streamActivities({ method: "item/completed", params: { item: spawn } })).toEqual([
      {
        kind: "output",
        text: JSON.stringify({
          status: "completed",
  ...
  test("projects opaque actor references through one isolated subsystem run", () => {
    const project = Subsystem.createActivityActorProjection()
    expect(
  ...
  test("keeps one native child label across its lifecycle, prose, tool and output", () => {
    ...
    const activities = events
      .flatMap((event) => Subsystem.codex.streamActivities(event))
      .map(project)
      .filter((activity): activity is Subsystem.PhaseActivity => activity !== undefined)
  ...
  test("maps agent, shell and MCP events into the subsystem-neutral live feed", () => {
    expect(
      Subsystem.codex.streamActivities({
        type: "item.started",
