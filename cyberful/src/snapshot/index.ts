...
// Tracks project changes in a private Git directory and provides bounded patch,
// restore, revert, diff, and cleanup operations for session workflows.
// → cyberful/src/session/revert.ts — restores user-selected session changes.
// → cyberful/src/git/index.ts — shares stable Git parsing and output conventions.
// ─────────────────────────────────────────────────────────────────
...
export const FileDiff = Schema.Struct({
  // Legacy summaries may omit file details and patch text, so those fields stay
  // optional at the storage boundary instead of invalidating the full session.
...
export class Service extends Context.Service<Service, Interface>()("@cyberful/Snapshot") {}

export const layer: Layer.Layer<
  Service,
  never,
  AppFileSystem.Service | AppProcess.Service | Config.Service | InstanceDisposalRegistry.Service
> = Layer.effect(
    Service,
    Effect.gen(function* () {
            ...
            return yield* locked(
              Effect.gen(function* () {
                ...
                const load = Effect.fnUntraced(
                  ...
                  Effect.catch((error) =>
                    Effect.sync(() => {
                      log.warn("snapshot batch read failed; using per-file Git reads", { error })
                      return undefined
                    }),
                  ),
                )
