...
// Runs bounded, restart-safe data backfills and records each completed name in
// SQLite before allowing future startups to skip it.
// → cyberful/src/data-migration.sql.ts — declares the completion marker table.
// ─────────────────────────────────────────────────────────────────────────────
...
export class Service extends Context.Service<Service, Interface>()("@cyberful/DataMigration") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      // ── Background Migrations Commit Their Own Resume Point ──────
      // Startup must not wait for historical data repair, but a failed repair
      // must remain visible and retryable. Each migration writes its completion
      // row only after its mutation succeeds, so interruption or failure leaves
      // it eligible for the next scoped runtime. The background fiber therefore
      // reports its complete cause and deliberately recovers without claiming
      // completion or failing the interactive application startup.
      // ─────────────────────────────────────────────────────────────────
      for (const migration of migrations) {
        ...
        Database.use((db) => db.insert(DataMigrationTable).values({ name: migration.name }).onConflictDoNothing().run())
      }
    }).pipe(
