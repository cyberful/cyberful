// ── Scoped Cross-Platform Process Spawner ──────────────────────────
// Adapts Effect command graphs to cross-spawn, wires typed streams and file
// descriptors, and terminates complete process groups when their scope closes.
// → cyberful/src/effect/process.ts — adds bounded application-level execution.
// ────────────────────────────────────────────────────────────────────
...
const flatten = (command: ChildProcess.Command) => {
  ...
  if (!head) throw new Error("flatten produced empty commands array")
  return { commands, head, tail: commands.slice(1), opts }
}
...
const DEFAULT_FORCE_KILL_AFTER = "5 seconds"
...
export const make = Effect.gen(function* () {
  ...
  const setupStdin = (
    ...
    Effect.suspend(() => {
      ...
      const writable = proc.stdin
      if (Predicate.isNotNull(writable)) {
        sink = NodeSink.fromWritable({
          evaluate: () => writable,
  ...
  const setupOutput = (
    ...
    const stderrReadable = proc.stderr
    let stdout = stdoutReadable
      ? NodeStream.fromReadable({
          evaluate: () => stdoutReadable,
          onError: (cause) => toPlatformError("fromReadable(stdout)", toError(cause), command),
        })
      : Stream.empty
    let stderr = stderrReadable
      ? NodeStream.fromReadable({
          evaluate: () => stderrReadable,
  ...
  const spawn = (command: ChildProcess.StandardCommand, opts: NodeChildProcess.SpawnOptions) =>
    Effect.callback<readonly [NodeChildProcess.ChildProcess, ExitSignal, number], PlatformError.PlatformError>(
      (resume) => {
        ...
        proc.on("spawn", () => {
          ...
          resume(
            pid === undefined
              ? Effect.fail(toPlatformError("spawn", new Error("Child process has no process ID"), command))
              : Effect.succeed([proc, signal, pid]),
          )
        })
        return Effect.sync(() => {
          proc.kill("SIGTERM")
  ...
  const killGroup = (
    ...
    const pid = proc.pid
    if (pid === undefined) {
      return Effect.fail(toPlatformError("kill", new Error("Child process has no process ID"), command))
    }
    ...
    function* (command) {
      switch (command._tag) {
        case "StandardCommand": {
          ...
          // ── Process Scope Owns Termination And Reaping ───────────────
          // Every spawned handle is acquired inside the caller's Effect scope.
          // Scope closure sends the configured graceful signal to the whole process
          // group, waits for the close event, then escalates to SIGKILL after a
          // bounded delay. Repeated cleanup observes the same deferred exit signal.
          // ─────────────────────────────────────────────────────────────────

          const [proc, signal, pid] = yield* Effect.acquireRelease(
            spawn(command, {
              cwd: dir,
            ...
            Effect.fnUntraced(function* ([proc, signal]) {
              ...
              if (done) {
                ...
                return yield* kill(killGroup).pipe(
                  Effect.catchIf(
        ...
        case "PipedCommand": {
          ...
          let handle = spawnCommand(flat.head)
          for (let i = 0; i < flat.tail.length; i++) {
            const next = flat.tail[i]
            if (!next) continue
