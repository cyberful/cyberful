// ── Serialized Interactive Work Runner ─────────────────────────────
// Coordinates one background run or interactive shell at a time, queues a run
// behind an active shell, and makes interruption and idle transitions explicit.
// → cyberful/src/session/prompt.ts — uses the runner for interactive shell work.
// ────────────────────────────────────────────────────────────────────
...
export const make = <A, E = never>(
  ...
  // ── Run Completion Changes Only Its Own Generation ────────────────
  // Each background run receives a monotonic identifier before it is forked into
  // the owning scope. Its finalizer may publish completion to every waiter, but
  // it returns the runner to Idle only while the same generation still owns state.
  // This prevents a late exit from overwriting cancellation or a successor run.
  // ─────────────────────────────────────────────────────────────────────
  ...
  // ── Cancellation Atomically Relinquishes Runner Ownership ─────────
  // State changes to Idle before interruption effects execute, preventing new
  // callers from joining work that is already being cancelled. The former owner
  // is then interrupted and its waiters receive RunnerCancelled exactly once.
  // Shell readiness is awaited so cancellation never races its startup handshake.
  // ─────────────────────────────────────────────────────────────────────
