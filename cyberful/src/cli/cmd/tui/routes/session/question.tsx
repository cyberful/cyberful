// ── Blocking Question Composer ───────────────────────────────────
// Collects single or multi-question answers, optional custom text, review, and
//   rejection while owning its temporary keymap mode and textarea cleanup.
// ─────────────────────────────────────────────────────────────────
...
import type { QuestionAnswer, QuestionRequest } from "@/server/client"
...
export function QuestionPrompt(props: { request: QuestionRequest }) {
  ...
  const reportFailure = (operation: string, error: unknown) => {
    log.warn(`question ${operation} failed`, { error, requestID: props.request.id })
  ...
  function submit() {
    ...
    observePromise(
      ...
      { rejected: (error) => reportFailure("reply", error) },
    )
  }
  ...
  function reject() {
    observePromise(
      ...
      { rejected: (error) => reportFailure("rejection", error) },
    )
  }
