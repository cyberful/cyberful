// ── Interactive Diff Viewer Feature ──────────────────────────────
// Loads working-tree or turn diffs, coordinates file-tree and patch selection,
//   review state, split or unified layout, keyboard commands, and route cleanup.
// ─────────────────────────────────────────────────────────────────
...
const PLAIN_TEXT_FILETYPE = "cyberful-plain-text"
...
function storedRoute(value: unknown): TuiRouteCurrent | undefined {
  if (!isRecord(value) || typeof value.name !== "string") return undefined
  if (value.params !== undefined && !isRecord(value.params)) return undefined
  return value.params === undefined ? { name: value.name } : { name: value.name, params: value.params }
}

function routeParams(route: TuiRouteCurrent) {
  const candidate: unknown = "params" in route ? route.params : undefined
  if (!isRecord(candidate)) return undefined
  const mode: DiffMode | undefined =
    candidate.mode === "git" || candidate.mode === "last-turn" ? candidate.mode : undefined
  ...
  const returnRoute = storedRoute(candidate.returnRoute)
  return { mode, sessionID, messageID, returnRoute }
}

function DiffViewer(props: { api: TuiFeatureApi }) {
  const dimensions = useTerminalDimensions()
  const themeState = useTheme()
  const theme = () => props.api.theme.current
  const params = () => routeParams(props.api.route.current)
  ...
  const currentPatchFileIndex = () => {
    ...
    if (!currentScroll) return undefined
    const viewportContentY = currentScroll.scrollTop + 1
    const entries = patchFileIndexes()
      ...
      .map((entry) => ({
        ...entry,
        contentY: currentScroll.scrollTop + entry.node.y - currentScroll.viewport.y,
      }))
  ...
  const measurePatchFiller = () => {
    requestAnimationFrame(() => {
      ...
      const contentHeight = Math.max(
        ...entries.map((node) => currentScroll.scrollTop + node.y - currentScroll.viewport.y + node.height),
      )
      setPatchFillerHeight(Math.max(0, currentScroll.viewport.height - contentHeight))
    })
  }
