// ── Contextual Key Guide Feature ─────────────────────────────────
// Groups active keymap bindings into a dock or overlay, previews pending leader
//   sequences, and persists layout and preview preferences through the feature API.
// ─────────────────────────────────────────────────────────────────
...
function ink(api: TuiFeatureApi, name: string, fallback: string): Color {
...
function skin(api: TuiFeatureApi): Skin {
  return {
...
function activeKeyEntry(api: TuiFeatureApi, active: ActiveKey<Renderable, KeyEvent>): Entry {
...
function commandShortcut(api: TuiFeatureApi, name: string) {
...
function HomeHint(props: { api: TuiFeatureApi }) {
  ...
  return (
    <box width="100%" maxWidth={75} alignItems="center" paddingTop={1} flexShrink={0}>
      <text fg={look().muted} wrapMode="none">
        <span style={{ fg: look().subtle }}>/workflow</span> to choose a workflow · Show keyboard shortcuts with{" "}
...
function WhichKeyPanel(props: {
  ...
  const moveGroup = (delta: number) => {
    ...
    const group = list[(index + delta + list.length) % list.length]
    if (!group) return
