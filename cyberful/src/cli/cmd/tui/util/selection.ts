...
// Extracts renderer selections, lets focused controls normalize copied text,
//   writes it to the clipboard, and reports success or failure by toast.
// ─────────────────────────────────────────────────────────────────
...
export function copy(renderer: Renderer, toast: Toast): boolean {
  ...
  observePromise(Clipboard.copy(clipboardText), {
    fulfilled: () => toast.show({ message: "Copied to clipboard", variant: "info" }),
    rejected: (error) => toast.error(error),
  })
