// ── TUI Keybinding Configuration ─────────────────────────────────
// Defines runtime schemas, defaults, command mappings, and normalized OpenTUI
//   bindings for every configurable terminal action.
// ─────────────────────────────────────────────────────────────────
...
export const Definitions = {
  ...
  theme_switch_mode: keybind("<leader>shift+t", "Switch between light and dark theme mode"),
  ...
  session_toggle_todo: keybind("<leader>t", "Toggle todo list"),
  session_toggle_findings: keybind("<leader>f", "Toggle findings sidebar"),
  ...
  // ── The First Binding Is Also The Visible Shortcut ──────────────
  // Shortcut formatting presents the first configured sequence in the jump-to-
  // bottom affordance. `ctrl+end` leads because the focused prompt owns plain
  // `end`; the modified sequence still reaches the session command while typing.
  // Alternative bindings remain active but do not replace that stable label.
  // ─────────────────────────────────────────────────────────────────
  messages_last: keybind("ctrl+end,ctrl+alt+g,end", "Navigate to last message"),
  ...
  prompt_queue: keybind("tab", "Queue prompt"),
...
export const Descriptions = mapValues(Definitions, (item) => item.description)
export const CommandMap = {
  ...
  session_toggle_todo: "session.toggle.todo",
  session_toggle_findings: "session.toggle.findings",
...
function isCommandName(name: string): name is keyof typeof CommandMap {
  return Object.hasOwn(CommandMap, name)
}

const CommandDescriptions: Record<string, string> = Object.fromEntries(
  Object.entries(Definitions).map(([name, item]) => [isCommandName(name) ? CommandMap[name] : name, item.description]),
)
...
const decodeBindingValueSchema = Schema.decodeUnknownSync(BindingValueSchema)

function decodeBindingValue(value: unknown): BindingValueSchema {
  // Effect Schema proves the complete binding union before this assertion; it
  // removes only the readonly projection that schema decoders expose by default.
  return decodeBindingValueSchema(value) as BindingValueSchema
}
...
export function parse(keybinds: KeybindOverrides): Keybinds {
  ...
  return mapValues(Definitions, (item, name) => decodeBindingValue(keybinds[name] ?? item.default))
}
