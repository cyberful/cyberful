...
// Filters, groups, scrolls, and selects typed options with configurable actions,
//   keybindings, mouse navigation, categories, footers, and current-value focus.
// ─────────────────────────────────────────────────────────────────
...
export function DialogSelect<T>(props: DialogSelectProps<T>) {
  ...
  // Filtering can move rows beneath a stationary pointer and synthesize mousemove.
  // Reset ownership before the relocated row can steal keyboard selection.
  createEffect(() => {
    filtered()
  ...
  return (
    <>
      ...
      <Show when={props.footer}>
        <box flexShrink={0}>
          <text fg={props.footerFg ?? (props.active ? fg : theme.textMuted)}>{props.footer}</text>
        </box>
      </Show>
    </>
  )
