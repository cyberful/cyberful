...
// Reconstructs reducer state, scrollback commits, prompt history, and the active
//   footer blocker from persisted messages and pending questions.
// ─────────────────────────────────────────────────────────────────

import type { Event, QuestionRequest } from "@/server/client"
...
function replayMessage(
  ...
  const info = apply(data, messageUpdated, message.info.sessionID, thinking, limits)
  ...
  const final = apply(data, messageUpdated, message.info.sessionID, thinking, limits)
  patch = mergePatch(patch, final.footer?.patch)
  commits.push(...final.commits)
