...
// Drives the real reducer, footer, question, and snapshot pipeline with local
//   synthetic control-plane events so interactive presentation can be previewed
//   without a server or model call; demo replies complete the matching tools.
// → cyberful/src/cli/cmd/run/session-data.ts — reduces the synthetic event feed.
// ─────────────────────────────────────────────────────────────────

import path from "node:path"
import type { Event, ToolPart } from "@/server/client"
import { createSessionData, reduceSessionData, type SessionData } from "./session-data"
import { writeSessionOutput } from "./stream"
import type { FooterApi, QuestionReject, QuestionReply, RunPrompt, StreamCommit } from "./types"
...
function take(state: State, key: "msg" | "part" | "call" | "ask", prefix: string): string {
  state[key] += 1
...
function open(state: State): string {
  ...
  feed(state, {
    id: `message.updated:${id}:${Date.now()}`,
...
async function emitText(state: State, body: string, signal?: AbortSignal): Promise<void> {
  ...
  feed(state, {
    id: `message.part.updated:${part}:${Date.now()}`,
  ...
  for (const item of split(body)) {
    ...
    next += item
    feed(state, {
      id: `message.part.delta:${part}:${Date.now()}`,
...
function startTool(state: State, ref: Ref, metadata: Record<string, unknown> = {}): void {
  feed(state, {
    id: `message.part.updated:${ref.part}:${Date.now()}`,
    type: "message.part.updated",
    properties: {
      sessionID: state.id,
      time: Date.now(),
...
function emitTask(state: State): void {
  ...
  const part = {
    ...
    state: {
      status: "running",
      input: {
        filePath: "cyberful/src/cli/cmd/run/stream.ts",
        offset: 1,
        limit: 200,
      },
      time: {
...
function emitTodo(state: State): void {
  const ref = make(state, "todowrite", {
    todos: [
      {
        content: "Inspect tool formatting",
        status: "completed",
      },
      {
      ...
      return [
        {
          ...
          options: [
            { label: "Transcript", description: "Inspect the transcript layout" },
...
export function createRunDemo(input: Input) {
  ...
  const questionReject = (input: QuestionReject): boolean => {
    ...
    state.asks.delete(input.requestID)
    feed(state, {
      id: `question.rejected:${input.requestID}:${Date.now()}`,
