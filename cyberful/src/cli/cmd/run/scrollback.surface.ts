// ── Retained Scrollback Streaming ────────────────────────────────
// Owns mutable render surfaces for in-progress assistant, reasoning, and tool
//   entries, then commits them into immutable scrollback without losing stable
//   Markdown or code layout as chunks arrive.
// ─────────────────────────────────────────────────────────────────
...
export class RunScrollbackStream {
  ...
  private async writeStreaming(commit: StreamCommit, body: ActiveBody): Promise<void> {
    ...
    this.active.content = commit.mode === "replace" ? body.content : this.active.content + body.content
