// ── Subagent Activity Reducer ────────────────────────────────────
// Builds bounded child-session tabs and detail snapshots from nested session
//   events while preserving each child reducer's independent state and blockers.
// ─────────────────────────────────────────────────────────────────

import type { Event, Message, Part, QuestionRequest, ToolPart } from "@/server/client"
...
export const SUBAGENT_DETAIL_BOOTSTRAP_LIMIT = 80
...
function queueChanged(data: SessionData, before: ReturnType<typeof queueSnapshot>) {
  return !sameQueue(before.questions, data.questions)
}
...
function inputLabel(input: Record<string, unknown>): string | undefined {
  ...
  const filePath = text(input.path) ?? text(input.filePath) ?? text(input.filepath)
  if (filePath) {
    return filePath
  }
...
function taskTab(part: ToolPart, sessionID: string): FooterSubagentTab {
  const label = Locale.titlecase(text(part.state.input.subagent_type) ?? "small-worker")
...
function mergeLiveCommit(current: StreamCommit, next: StreamCommit) {
  ...
  const merged = {
    ...current,
    ...next,
    text: next.mode === "replace" ? next.text : current.text + next.text,
  }
...
function ensureBlockerTab(data: SubagentData, sessionID: string, title: string | undefined) {
  const description = "Pending question"
...
export function bootstrapSubagentData(input: BootstrapSubagentInput) {
  ...
  for (const item of input.questions) {
    ...
    changed = ensureBlockerTab(input.data, item.sessionID, child.get(item.sessionID)?.title) || changed
  }
...
export function bootstrapSubagentCalls(input: {
  ...
  const tabChanged = input.messages
    .flatMap((message) => message.parts)
    .filter((part): part is ToolPart => part.type === "tool" && part.sessionID === input.sessionID)
    .reduce((updated, part) => syncTaskTab(input.data, part) || updated, false)

  return changed || tabChanged || queueChanged(detail.data, before)
}
...
export function reduceSubagentData(input: {
  ...
  if (event.type === "message.part.updated") {
    ...
    if (part.type === "tool" && part.tool === "task" && knownSession(input.data, part.sessionID)) {
      const detail = ensureDetail(input.data, part.sessionID)
      const changed = applyChildEvent({
        detail,
        event,
        thinking: input.thinking,
        limits: input.limits,
      ...
      return syncTaskTab(input.data, part) || changed
    }
  }
