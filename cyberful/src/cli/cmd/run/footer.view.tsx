// ── Split-Footer Layout ──────────────────────────────────────────
// Projects footer signals into prompt, question, command, skill, and subagent
//   panels plus the status row; it owns only derived view state and sizing.
// → cyberful/src/cli/cmd/run/footer.ts — owns the mutable footer state.
// ─────────────────────────────────────────────────────────────────
...
import { RunPromptBody, TEXTAREA_MIN_ROWS, createPromptState, hintFlags } from "./footer.prompt"
...
export function RunFooterView(props: RunFooterViewProps) {
  ...
  const panel = createMemo(() => selectingSubagent() || commanding() || skilling())
  ...
  const spinnerColor = createMemo(() => bouncingBarColors(theme().muted, theme().highlight))
  ...
  createEffect(() => {
    ...
    elapsedTimer = setInterval(() => setElapsedNow(Date.now()), 1000)
  })
  onCleanup(() => {
  ...
  const elapsed = createMemo(() => {
    ...
    if (typeof start === "number") {
      return Locale.clockDuration(elapsedNow() - start)
    }
  ...
  const [composerRows, setComposerRows] = createSignal(TEXTAREA_MIN_ROWS)
  const composer = createPromptState({
    ...
    onSkillMenu: openSkill,
    onRows: setComposerRows,
  ...
  createEffect(() => {
    ...
    const current = route()
    if (current.type !== "command" && current.type !== "skill" && current.type !== "subagent-menu") {
      return
    }
              ...
              >
                ...
                <Show when={!menu() && !panel()}>
                  ...
                  >
                    ...
                    <Show when={!shell()}>
                      <text id="run-direct-footer-elapsed" fg={theme().muted} wrapMode="none" truncate flexShrink={0}>
                        · {elapsed()}
                      </text>
                    </Show>
                  </box>
              ...
              <Show
                ...
                fallback={
                  <box
                    ...
                    flexShrink={0}
                  >
                    <Show when={busy() || exiting() || subagentIndicator()}>
                      <box
                        id="run-direct-footer-hint-left"
                        flexDirection="row"
                        gap={1}
                        flexGrow={1}
                        flexShrink={1}
                      ...
                      >
                        ...
                        <Show when={busy() && !exiting()}>
                          ...
                          <Show when={status()}>
                            {(message) => (
                              <RunGenerationStatusText
                                id="run-direct-footer-status-label"
                                message={message()}
                                theme={theme}
                              />
                            )}
                          ...
                          <text id="run-direct-footer-hint-interrupt" fg={theme().text} wrapMode="none" truncate>
                            {interruptKey()} <span style={{ fg: theme().muted }}>cancel</span>
                          </text>
                        </Show>
...
function RunGenerationStatusText(props: { id: string; message: string; theme: () => RunTheme["footer"] }) {
  ...
  return (
    <text id={props.id} fg={props.theme().muted} wrapMode="none" truncate flexShrink={1}>
      <Show when={status()} fallback={props.message}>
        {(value) => (
          <>
            generating... <span style={{ fg: props.theme().shade }}>{value().leadingZeros}</span>
            <span style={{ fg: props.theme().text }}>{value().tokenDigits}</span>
            {value().suffix}
          </>
        )}
      </Show>
