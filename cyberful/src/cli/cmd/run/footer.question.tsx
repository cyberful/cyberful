// ── Footer Question Interaction ──────────────────────────────────
// Renders single and multi-question flows, custom answers, tabbed review, and
//   keyboard dispatch while the pure question state machine owns transitions.
// → cyberful/src/cli/cmd/run/question.shared.ts — validates and advances answers.
// ─────────────────────────────────────────────────────────────────
...
import type { QuestionRequest } from "@/server/client"
import {
...
const log = Log.create({ service: "run.footer.question" })
...
export function RunQuestionBody(props: {
  ...
  let submission: { requestID: string; task: Promise<void> } | undefined
  let mounted = true
  ...
  // ── One Question Owns One Submission Task ───────────────────────
  // Keyboard and mouse input can reach the same action in adjacent render turns.
  // A retained task joins that work to this component and prevents duplicate
  // replies while the control plane is accepting the first. Successful delivery
  // remains visibly pending until its event removes the question; failure may
  // unlock only the still-mounted request that originally owned the operation.
  // ─────────────────────────────────────────────────────────────────
  const beginSubmission = (requestID: string, label: string, send: () => void | Promise<void>) => {
    if (submission?.requestID === requestID) return
    setState((prev) => questionSetSubmitting(prev, true))
    let task: Promise<void>
    task = Promise.resolve()
      .then(send)
      .catch((error) => {
        log.warn(`question ${label} failed`, { error, requestID })
        if (mounted && state().requestID === requestID) {
          setState((prev) => questionSetSubmitting(prev, false))
        }
      })
      .finally(() => {
        if (submission?.task === task) submission = undefined
      })
    submission = { requestID, task }
  }
  ...
  const beginReject = (input: QuestionReject) => {
    beginSubmission(input.requestID, "rejection", () => props.onReject(input))
  }
