...
// Owns the direct-mode textarea state, keybindings, prompt history, and file or
//   subagent mention completion rendered inside the mutable footer.
// → cyberful/src/cli/cmd/run/prompt.shared.ts — supplies pure prompt transitions.
// ─────────────────────────────────────────────────────────────────
...
import type { FooterKeybinds, FooterState, RunCommand, RunPrompt, RunPromptPart } from "./types"

const AUTOCOMPLETE_ROWS = FOOTER_MENU_ROWS
const AUTOCOMPLETE_BOTTOM_ROWS = 1
const log = Log.create({ service: "run.footer.prompt" })
...
type Mention = Extract<RunPromptPart, { type: "file" }>
  ...
  const refreshPasteLayout = () => {
    ...
    pasteTick = setTimeout(() => {
      ...
      observePromise(
        ...
        {
          rejected: (error) => log.debug("renderer did not settle after paste", { error }),
        },
      )
    }, 0)
  }
...
export function createPromptState(input: PromptInput): PromptState {
  ...
  const mentionOptions = createMemo(() => files())
  const slashOptions = createMemo<SlashOption[]>(() => {
    const builtins = [
      { kind: "slash", name: "new", display: "/new", description: "start a new session" } satisfies SlashOption,
      { kind: "slash", name: "skill", display: "/skill", description: "select a skill" } satisfies SlashOption,
  ...
  const syncParts = () => {
    ...
    for (const item of area.extmarks.getAllForTypeId(type)) {
      ...
      const prev = part.source?.text.value ?? "@" + (part.filename ?? "")
      if (text !== prev) {
        continue
      }
  ...
  const restoreParts = (value: RunPromptPart[]) => {
    clearParts()
    parts = value.filter((item): item is Mention => item.type === "file").map((item) => structuredClone(item))
    if (!area || area.isDestroyed || type === 0) {
      return
    }
    ...
    parts.forEach((item, idx) => {
      const start = item.source?.text.start
      const end = item.source?.text.end
  ...
  const submitPrompt = (next: RunPrompt) => {
    ...
    if (next.mode !== "shell" && slashHead(next.text)?.name === "skill") {
      input.onSkillMenu?.()
      return
    }
