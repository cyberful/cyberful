// ── Immutable Scrollback Writers ─────────────────────────────────
// Converts completed commits into grouped OpenTUI writers for text, code,
//   diffs, tasks, todos, timestamps, and separators in terminal history.
// ─────────────────────────────────────────────────────────────────
...
export function entryLayout(commit: StreamCommit, body: RunEntryBody = entryBody(commit)): EntryLayout {
  if (commit.kind === "tool") {
    if (body.type === "structured" || body.type === "markdown" || body.type === "code") {
      return "block"
    }
...
export function RunEntryContent(props: {
  ...
  const assistantMarkdown = createMemo(() => {
    ...
    if (props.commit.kind !== "assistant") return { text: next.content, timestamp: undefined }
    return splitAssistantTimeLine(next.content)
  })
  ...
  return (
    <Switch fallback={null}>
            ...
            <box width="100%" paddingLeft={1}>
              <line_number width="100%" fg={theme().block.muted} minWidth={3} paddingRight={1}>
                <code
                  width="100%"
                  wrapMode="char"
                  filetype={toolFiletype(current().file)}
                  streaming={false}
                  syntaxStyle={syntax()}
                  content={current().content}
                  fg={theme().block.text}
      ...
      <Match when={assistantMarkdown()}>
        {(current) => (
          <box width="100%" flexDirection="column">
            <Show when={current().text.trim()}>
              <markdown
                width="100%"
                syntaxStyle={syntax()}
                streaming={streaming()}
            ...
            <Show when={current().timestamp}>
              {(value) => (
                <text width="100%" wrapMode="word" marginTop={current().text.trim() ? 1 : 0} fg={theme().block.muted}>
