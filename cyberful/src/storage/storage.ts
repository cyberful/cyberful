// ── Legacy File Storage Service ────────────────────────────────────────────
// Migrates and serves the remaining JSON-backed session artifacts with
// per-path read/write locking and explicit missing-resource failures.
// → cyberful/src/storage/json-migration.ts — imports this legacy layout into SQLite.
// ──────────────────────────────────────────────────────────────────────
...
export interface Interface {
  ...
  readonly read: (key: string[]) => Effect.Effect<unknown, Error>
  readonly update: (key: string[], fn: (draft: unknown) => void) => Effect.Effect<unknown, Error>
...
export class Service extends Context.Service<Service, Interface>()("@cyberful/Storage") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const read: Interface["read"] = (key) =>
      withResolved(key, (target, rw) => TxReentrantLock.withReadLock(rw, wrap(target, fs.readJson(target))))

    const update: Interface["update"] = (key, fn) =>
      Effect.gen(function* () {
    ...
    const list: Interface["list"] = Effect.fn("Storage.list")(function* (prefix: string[]) {
      ...
      const result = yield* fs
        ...
        .pipe(Effect.catchIf(missing, () => Effect.succeed<string[]>([])))
      return result
