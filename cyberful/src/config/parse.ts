// ── JSONC Configuration Validation ───────────────────────────────
// Parses JSON with comments and trailing commas, reports source positions, rejects
// unknown top-level keys, and decodes the result through an Effect schema.
// → cyberful/src/config/error.ts — carries parse and schema issues to callers.
// → cyberful/src/config/config.ts — supplies the canonical configuration schema.
// ─────────────────────────────────────────────────────────────────
...
export function schema<S extends EffectSchema.Decoder<unknown, never>>(
  ...
  // ── Decoding Proves The Mutable Configuration View ──────────────
  // Effect Schema has validated the complete unknown value as `S.Type` before
  // this branch is entered. Configuration assembly needs a mutable projection
  // because later precedence layers merge into it, but DeepMutable changes only
  // TypeScript's readonly markers and never changes the decoded runtime value.
  // The assertion therefore grants mutation after validation, not before it.
  // ─────────────────────────────────────────────────────────────────
