...
export interface Interface {
  ...
  // ── Subscriptions Acquire Before Returning ──────────────────────
  // subscribe and subscribeAll acquire their PubSub subscription in the caller's
  // Scope at yield time, before the returned stream can be consumed. Every publish
  // after that yield is therefore visible even when the consumer starts pulling
...
export class Service extends Context.Service<Service, Interface>()("@cyberful/Bus") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // ── Typed Channels Share One Runtime Payload Shape ────────────
    // The map is heterogeneous at compile time but keyed by the exact event type
    // that selects each definition-specific channel. Every stored PubSub carries
    // the common Payload representation; the assertion restores only the generic
        ...
        yield* Scope.provide(scope)(
          Stream.fromSubscription(subscription).pipe(
            Stream.runForEach((msg) =>
              ...
              }).pipe(Effect.catch((cause) => Effect.sync(() => log.error("subscriber failed", { type, cause })))),
            ),
            Effect.forkScoped,
          ),
        )
...
export const defaultLayer = layer.pipe(Layer.provide(InstanceDisposalRegistry.layer))

const { runPromise, runSync } = makeRuntime(Service, defaultLayer)

// ── Callback Setup Is Deliberately Synchronous ────────────────────
// Callback registration must return an unsubscribe function before the caller can
// publish. Instance lookup, PubSub subscription, Scope creation, and scoped fork
// are all synchronous Effect operations today, so runSync preserves that contract.
// If one of those operations becomes asynchronous, setup fails loudly instead of
// returning before ownership of the subscription has been established.
// ─────────────────────────────────────────────────────────────────
