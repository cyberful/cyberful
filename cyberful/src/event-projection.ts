// ── Transactional Event Projection ───────────────────────────────
...
// → cyberful/src/event-publisher.ts — routes versioned events onto this projection surface.
// ─────────────────────────────────────────────────────────────────
import { Database } from "@/storage/db"
import { GlobalBus } from "@/bus/global"
import { Bus as ProjectBus } from "@/bus"
import { EventID } from "./event-projection-schema"
...
export class Service extends Context.Service<Service, Interface>()("@cyberful/EventProjection") {}

export const layer = Layer.effect(Service)(
  Effect.gen(function* () {
    const bus = yield* ProjectBus.Service

    const run: Interface["run"] = Effect.fn("EventProjection.run")(function* (def, data, options) {
      const agg = isRecord(data) ? data[def.aggregate] : undefined
      if (typeof agg !== "string") {
        throw new Error(`EventProjection.run: "${def.aggregate}" required but not found: ${JSON.stringify(data)}`)
      }

      if (def.version !== versions.get(def.type)) {
        throw new Error(`EventProjection.run: running old versions of events is not allowed: ${def.type}`)
      }
...
export function init(input: { projectors: Array<[Definition, ProjectorFunc]>; convertEvent?: ConvertEvent }) {
  ...
  for (const entry of EventDefinition.definitions()) {
...
function process<Def extends Definition>(
  ...
  if (projectors == null) {
    throw new Error("No projectors available. Call `EventProjection.init` to install projectors")
  }
...
export function effectPayloads() {
  return [
    ...registry
      ...
      .toArray(),
    ...EventDefinition.definitions().flatMap((definition) => {
...
export * as EventProjection from "./event-projection"
