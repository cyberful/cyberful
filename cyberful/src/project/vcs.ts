...
// Computes bounded Git status, diffs, and patches for the active project instance.
// → cyberful/src/git/index.ts — executes the underlying Git operations.
// ──────────────────────────────────────────────────────────────────────────
...
export const FileDiff = Schema.Struct({
  file: Schema.String,
  // ── VCS Diffs Match Snapshot Compatibility ──────────────────────────────────
  // Current Git producers include a patch, but the public shape mirrors the
  // sibling Snapshot.FileDiff contract where patch remains optional. Keeping
  // both schemas compatible prevents a future metadata-only diff producer from
  // breaking the instance endpoint. Callers must therefore render the patch
  // only when it is present rather than asserting today’s producer behavior.
  // ───────────────────────────────────────────────────────────────────────────
...
export class Service extends Context.Service<Service, Interface>()("@cyberful/Vcs") {}
