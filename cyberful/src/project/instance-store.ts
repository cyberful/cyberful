// ── Scoped Project Instance Store ─────────────────────────────────────────
// Deduplicates project loads and owns each instance context, scope, bootstrap, and disposal.
// → cyberful/src/project/instance-context.ts — defines the context exposed to consumers.
// → cyberful/src/project/bootstrap-service.ts — initializes a newly loaded instance.
// ───────────────────────────────────────────────────────────────────────
...
export class Service extends Context.Service<Service, Interface>()("@cyberful/InstanceStore") {}
...
export const layer: Layer.Layer<
  Service,
  never,
  Project.Service | InstanceBootstrap.Service | InstanceDisposalRegistry.Service
> = Layer.effect(
  Service,
  Effect.gen(function* () {
    const project = yield* Project.Service
    const bootstrap = yield* InstanceBootstrap.Service
    const disposalRegistry = yield* InstanceDisposalRegistry.Service
    ...
    const disposeContext = Effect.fn("InstanceStore.disposeContext")(function* (ctx: InstanceContext) {
      ...
      yield* Effect.promise(() => disposalRegistry.dispose(ctx.directory))
          ...
          yield* Effect.gen(function* () {
            ...
            if (previous) {
              ...
              if (Exit.isFailure(previousExit)) {
                yield* Effect.logWarning("replacing failed instance during reload").pipe(
                  Effect.annotateLogs({ directory, cause: previousExit.cause }),
                )
              }
              yield* Effect.promise(() => disposalRegistry.dispose(directory))
