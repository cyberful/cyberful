...
// → cyberful/builtin/skills/operate-nuclei/SKILL.md — contains the workflow under test.
// ─────────────────────────────────────────────────────────────────
...
test("the Nuclei skill preserves caller-controlled scan capability", async () => {
  const skill = await readFile(path.join(Builtin.DIR, "skills", "operate-nuclei", "SKILL.md"), "utf8")
