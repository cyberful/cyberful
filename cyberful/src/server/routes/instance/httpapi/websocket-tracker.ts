// ── Active WebSocket Shutdown Registry ──────────────────────────
// Serializes socket registration against shutdown and closes every tracked socket
// with a bounded wait before a listener scope is released.
// → cyberful/src/server/server.ts — invokes closeAll during listener shutdown.
// ─────────────────────────────────────────────────────────────────
...
export class Service extends Context.Service<Service, Interface>()("@cyberful/HttpApiWebSocketTracker") {}
      ...
      yield* Effect.all(
        active.map((close) =>
          close.pipe(
            ...
            Effect.catchCause((cause) => Effect.logWarning("websocket close failed", { cause })),
          ),
        ),
