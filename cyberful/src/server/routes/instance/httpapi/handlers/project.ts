...
// Lists, resolves, and updates projects through the active project service and
// marks the instance for reload when project metadata changes its runtime context.
// → cyberful/src/server/routes/instance/httpapi/groups/project.ts — defines project contracts.
// ─────────────────────────────────────────────────────────────────
...
export const projectHandlers = HttpApiBuilder.group(InstanceHttpApi, "project", (handlers) =>
  Effect.gen(function* () {
    ...
    const initGit = Effect.fn("ProjectHttpApi.initGit")(function* () {
      ...
      const next = yield* svc.initGit({ directory: ctx.directory, project: ctx.project }).pipe(Effect.orDie)
