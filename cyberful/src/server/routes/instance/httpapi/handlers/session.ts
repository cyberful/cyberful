...
// Implements session lifecycle, prompts, messages, commands, summaries,
// sharing, reverts, and streaming while translating failures at the HTTP boundary.
...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const createRaw = Effect.fn("SessionHttpApi.createRaw")(function* (ctx: {
      ...
      return yield* create({ payload: decoded })
    })
    ...
    const updatePart = Effect.fn("SessionHttpApi.updatePart")(function* (ctx: {
      ...
      yield* requireSession(ctx.params.sessionID)
      const payload = MessageV2.decodePart(ctx.payload)
      if (
