# ── Kubernetes Policy Campaign Tests ───────────────────────────
...
# → cyberful/builtin/skills/audit-kubernetes-policy-enforcement/scripts/run_kubernetes_policy_campaign.py — implementation.
# ─────────────────────────────────────────────────────────────────
...
SCRIPT = SKILL_ROOT / "scripts" / "run_kubernetes_policy_campaign.py"


def load_script() -> ModuleType:
    specification = importlib.util.spec_from_file_location("run_kubernetes_policy_campaign", SCRIPT)
    if specification is None or specification.loader is None:
        raise RuntimeError("could not load Kubernetes policy campaign")
...
class KubernetesPolicyCampaignTests(unittest.TestCase):
    def setUp(self) -> None:
        self.module = load_script()

    def _fixture(self, workspace: Path) -> tuple[Path, dict[str, object], bytes]:
        manifests = workspace / "manifests"
        policies = workspace / "policy"
        manifests.mkdir()
        policies.mkdir()
        (manifests / "deployment.yaml").write_text("apiVersion: apps/v1\nkind: Deployment\n", encoding="utf-8")
        (policies / "deny.rego").write_text("package main\ndeny contains msg if { false; msg := \"fixture\" }\n", encoding="utf-8")
        fake = workspace / "fake_conftest.py"
        fake.write_text(
            textwrap.dedent(
                """\
                #!/usr/bin/env python3
                ...
                if "--version" in sys.argv:
                    print("Conftest fixture 1.0")
                else:
                    print(json.dumps([{"filename": "deployment.yaml", "failures": [], "secret_leaked": bool(os.environ.get("CYBERFUL_TEST_SECRET")), "proxy_leaked": bool(os.environ.get("HTTP_PROXY"))}], sort_keys=True))
                """
            ),
            encoding="utf-8",
        )
        ...
        payload: dict[str, object] = {
            "$schema": "./kubernetes-policy-campaign.schema.json",
            "campaign_id": "fixture-kubernetes-policy",
            "scope_reference": "scope:K8S-1",
    ...
    def test_offline_campaign_preserves_raw_evidence_and_is_deterministic(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(os.environ, {"CYBERFUL_TEST_SECRET": "must-not-leak", "HTTP_PROXY": "http://127.0.0.1:9"}, clear=False):
            ...
            self.assertFalse(native[0]["secret_leaked"])
            self.assertFalse(native[0]["proxy_leaked"])
            self.assertEqual(first["command"][0], "conftest")
            self.assertEqual(first["tool"]["version_probe"]["argv"], ["conftest", "--version"])
            self.assertEqual(first["tool"]["version_probe"]["exit_code"], 0)
            self.assertIn(b"Conftest fixture 1.0", base64.b64decode(first["tool"]["version_probe"]["stdout"]["base64"]))
            self.assertEqual(first["network_sandbox"]["network"], "none")
            self.assertEqual(first["manifests"]["files"] + first["policies"]["files"], 2)
            self.assertEqual((workspace / "first.json").stat().st_mode & 0o777, 0o600)

    def test_refuses_missing_authority_and_insufficient_combined_limit_before_execution(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            workspace = Path(directory)
            fake, payload, raw = self._fixture(workspace)
            ...
            payload["scope_reference"] = "scope:K8S-1"
            payload["max_files"] = 1
            with self.assertRaisesRegex(self.module.CampaignError, "capacity"):
                self.module._run_campaign_for_test(payload, raw, workspace, workspace / "refused.json", str(fake))
            self.assertFalse(sentinel.exists())
    ...
    def test_tool_reads_content_snapshots_after_source_swap_and_rejects_symlinks(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            workspace = Path(directory)
            fake, payload, raw = self._fixture(workspace)
            source_file = workspace / "manifests" / "deployment.yaml"
            ...
            fake.write_text(
                textwrap.dedent(
                    ...
                    if "--version" in sys.argv:
                        pathlib.Path({str(source_file)!r}).write_text("kind: SwappedAfterSnapshot\\n", encoding="utf-8")
                        print("fixture 1")
                    else:
                        root = pathlib.Path(sys.argv[2])
                        print(json.dumps({{"snapshot": (root / "deployment.yaml").read_text(encoding="utf-8")}}))
                    """
                ),
                encoding="utf-8",
            )
            ...
            self.assertEqual(source_file.read_text(encoding="utf-8"), "kind: SwappedAfterSnapshot\n")

        with tempfile.TemporaryDirectory() as directory:
            workspace = Path(directory)
            fake, payload, raw = self._fixture(workspace)
            outside = workspace / "outside.yaml"
            outside.write_text("kind: Outside\n", encoding="utf-8")
            (workspace / "manifests" / "linked.yaml").symlink_to(outside)
    ...
    def test_cli_rejects_input_output_collision_without_mutation(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(payload["campaign_id"], "fixture-kubernetes-policy")
