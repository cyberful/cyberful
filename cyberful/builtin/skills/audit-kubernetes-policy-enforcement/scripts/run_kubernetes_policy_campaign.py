#!/usr/bin/env python3
# ── Offline Kubernetes Policy Campaign ─────────────────────────
# Evaluates confined local manifests with a fixed Conftest command and keeps
# bounded raw evidence without inheriting network routes or host credentials.
# → cyberful/builtin/skills/audit-kubernetes-policy-enforcement/scripts/manifest.json — execution contract.
# → cyberful/builtin/skills/audit-kubernetes-policy-enforcement/assets/kubernetes-policy-campaign.schema.json — input contract.
# → cyberful/builtin/skills/audit-kubernetes-policy-enforcement/tests/test_run_kubernetes_policy_campaign.py — refusal and execution coverage.
# ─────────────────────────────────────────────────────────────────
...
TRUSTED_COMMAND: Final = "conftest"
CONFIG_FIELDS: Final = frozenset({
    "$schema", "campaign_id", "scope_reference", "manifest_directory",
    "policy_directory", "max_files", "max_total_bytes", "timeout_seconds",
    "stdout_limit_bytes",
})
...
def _directory(workspace: Path, value: Any, label: str) -> tuple[str, Path]:
    relative = _text(value, label, 1024)
    resolved = _confined(workspace, relative, must_exist=True)
    metadata = resolved.lstat()
    if resolved.is_symlink() or not stat.S_ISDIR(metadata.st_mode):
        raise CampaignError(f"{label} must be a real directory")
    return relative, resolved


def _validated(payload: dict[str, Any], workspace: Path) -> tuple[str, str, Path, Path, int, int, int, int]:
    if set(payload) != CONFIG_FIELDS:
        raise CampaignError(f"input fields must be exactly {', '.join(sorted(CONFIG_FIELDS))}")
    if payload["$schema"] != "./kubernetes-policy-campaign.schema.json":
    ...
    _, manifests = _directory(workspace, payload["manifest_directory"], "manifest_directory")
    _, policies = _directory(workspace, payload["policy_directory"], "policy_directory")
    if manifests == policies:
        raise CampaignError("manifest_directory and policy_directory must be distinct")
    ...
    return campaign_id, scope_reference, manifests, policies, max_files, max_bytes, timeout, stdout_limit
...
def _snapshot(workspace: Path, root: Path, destination: Path, max_files: int, max_bytes: int, deadline: float) -> dict[str, Any]:
    ...
    for directory, directory_names, file_names in os.walk(root, topdown=True, followlinks=False):
        ...
        for name in file_names:
            ...
            entries.append((candidate.relative_to(root).as_posix(), metadata.st_size, _copy_regular_file(candidate, snapshot_directory / name, metadata, deadline)))
    if not entries:
        raise CampaignError(f"{root.name} must contain at least one regular file")
    digest = hashlib.sha256()
    ...
    return {"path": root.relative_to(workspace).as_posix(), "files": len(entries), "bytes": total_bytes, "sha256": digest.hexdigest()}
...
def _campaign(
    ...
    campaign_id, scope_reference, manifests, policies, max_files, max_bytes, timeout, stdout_limit = _validated(payload, workspace)
    started = time.monotonic()
    deadline = started + timeout
    execution_deadline = max(started, deadline - REPORT_RESERVE_SECONDS)
    for source in (manifests, policies):
        try:
            output.relative_to(source)
        except ValueError:
            continue
        raise CampaignError("output must not be inside an input directory")
    recorded_command = [TRUSTED_COMMAND, "test", "<snapshot>/manifests", "--policy", "<snapshot>/policy", "--output", "json", "--no-color"]
    sandbox_mechanism = sandbox_factory([command, "--version"]).mechanism
    with tempfile.TemporaryDirectory(prefix="cyberful-k8s-policy-") as campaign_directory:
        campaign_root = Path(campaign_directory)
        home = campaign_root / "home"
        home.mkdir(mode=0o700)
        snapshot_root = campaign_root / "snapshot"
        manifest_snapshot = snapshot_root / "manifests"
        policy_snapshot = snapshot_root / "policy"
        try:
            snapshot_root.mkdir(mode=0o700)
            manifest_inventory = _snapshot(workspace, manifests, manifest_snapshot, max_files, max_bytes, execution_deadline)
            remaining_files = max_files - manifest_inventory["files"]
            remaining_bytes = max_bytes - manifest_inventory["bytes"]
            if remaining_files < 1 or remaining_bytes < 1:
                raise CampaignError("combined inputs leave no capacity for policy_directory")
            policy_inventory = _snapshot(workspace, policies, policy_snapshot, remaining_files, remaining_bytes, execution_deadline)
            ...
            executed_command = [command, "test", str(manifest_snapshot), "--policy", str(policy_snapshot), "--output", "json", "--no-color"]
            ...
            _unlock_snapshot(snapshot_root)
    evidence = {
        "format": "cyberful.kubernetes-policy-evidence.v1",
        ...
        "interpretation": "Raw policy output is evidence to review; a denial or nonzero exit is not independently a verified vulnerability.",
    }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run a bounded offline Kubernetes policy audit")
