# ── Realtime Probe Runner Tests ─────────────────────────────────
# Exercises preflight authority and request limits, bounded process cleanup,
#   and a real authorized loopback webhook without any external target traffic.
# → cyberful/builtin/skills/test-realtime-integrations/scripts/run_realtime_probe.py — implementation.
# ─────────────────────────────────────────────────────────────────
...
SCRIPT = SKILL_ROOT / "scripts" / "run_realtime_probe.py"


def load_script() -> ModuleType:
    specification = importlib.util.spec_from_file_location("run_realtime_probe", SCRIPT)
    if specification is None or specification.loader is None:
        raise RuntimeError("could not load realtime probe runner")
...
class RecordingHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        response = json.dumps({"accepted": True, "echo": secret}).encode("utf-8")
...
def probe_plan(origin: str, *, allowed_origin: str | None = None, max_requests: int = 1, probes: int = 1, rate: float = 4) -> dict[str, Any]:
    entries = []
    for index in range(probes):
        entries.append({
            "probe_id": f"probe-{index}",
            "kind": "webhook",
            "principal": "controlled-sender",
            "tenant": "tenant-a",
            "channel": "orders",
            "method": "POST",
            "url": f"{origin}/hooks/{index}",
            "headers": {"Content-Type": "application/json"},
            "secret_headers": {},
            "body_base64": "eyJldmVudF9pZCI6InN5bnRoZXRpYyJ9",
        })
    return {
        "$schema": "./realtime-probe.schema.json",
        "campaign_id": "loopback-realtime",
        "authority": {
...
class RealtimeProbeTests(unittest.TestCase):
    ...
    def test_authorized_loopback_webhook_preserves_bounded_raw_evidence(self) -> None:
        process, evidence = self._run_cli(probe_plan(self.origin))

        self.assertEqual(process.returncode, 0, process.stderr)
        self.assertIsNotNone(evidence)
        assert evidence is not None
        self.assertEqual(RecordingHandler.requests, [("/hooks/0", b'{"event_id":"synthetic"}')])
        self.assertEqual(evidence["probes"][0]["transport"]["http_status"], 202)
        self.assertEqual(evidence["probes"][0]["transport"]["route"], "direct-loopback")
        self.assertFalse(evidence["probes"][0]["response"]["body_truncated"])
        self.assertNotIn("vulnerability", json.dumps(evidence["probes"][0]).lower())
    ...
    def test_authority_and_request_limits_reject_before_network(self) -> None:
        ...
        authority_process, authority_evidence = self._run_cli(probe_plan(self.origin, allowed_origin=wrong_origin))
        ...
        path_process, path_evidence = self._run_cli(probe_plan(self.origin, allowed_origin=f"{self.origin}/narrow"))
        ...
        limit_process, limit_evidence = self._run_cli(probe_plan(self.origin, max_requests=1, probes=2))
        ...
        localhost_process, localhost_evidence = self._run_cli(probe_plan(f"http://localhost:{self.server.server_port}"))
        self.assertEqual(localhost_process.returncode, 2)
        self.assertIsNone(localhost_evidence)
        self.assertIn("literal IP address", localhost_process.stderr)
    ...
    def test_json_cannot_select_transport_or_declare_authority(self) -> None:
        for name, value in (("confirmed", True), ("proxy_url", "http://127.0.0.1:8080"), ("ca_bundle", "ca.pem")):
            payload = probe_plan(self.origin)
    ...
    def test_runtime_route_accepts_docker_hostname_and_no_route_refuses_zero_connection(self) -> None:
        payload = probe_plan("http://target.example.invalid:80", allowed_origin="http://target.example.invalid:80")
        with tempfile.TemporaryDirectory() as directory:
            ca_bundle = Path(directory) / "runtime-ca.pem"
            ca_bundle.write_text("synthetic runtime trust", encoding="utf-8")
            _, _, probes = self.module._probe_plan(payload, Path(directory))
            with patch.dict(os.environ, {"HTTP_PROXY": "http://cyberful-os.invalid:8080", "CURL_CA_BUNDLE": str(ca_bundle)}, clear=True):
                route = self.module._runtime_route(probes)
            self.assertEqual(route.http_proxy, "http://cyberful-os.invalid:8080")
            with patch.dict(os.environ, {}, clear=True), patch.object(self.module, "_execute_probe") as execute:
                with self.assertRaisesRegex(self.module.ProbeError, "host-provided HTTP_PROXY"):
                    self.module.run_probe(payload, Path(directory), "synthetic-sha")
                execute.assert_not_called()
            with patch.dict(os.environ, {"HTTP_PROXY": "http://cyberful-os.invalid:8080"}, clear=True), patch.object(self.module, "_execute_probe") as execute:
                with self.assertRaisesRegex(self.module.ProbeError, "CA_BUNDLE or SSL_CERT_FILE"):
                    self.module.run_probe(payload, Path(directory), "synthetic-sha")
                execute.assert_not_called()
        self.assertEqual(RecordingHandler.requests, [])
    ...
    def test_sensitive_and_declared_secret_headers_reject_before_network(self) -> None:
        for name in ("Authorization", "Cookie", "Proxy-Authorization"):
            payload = probe_plan(self.origin)
            payload["probes"][0]["headers"] = {name: "inline-secret"}
            process, evidence = self._run_cli(payload)
            self.assertEqual(process.returncode, 2)
            self.assertIsNone(evidence)
            self.assertIn("forbidden header", process.stderr)
        payload = probe_plan(self.origin)
        payload["probes"][0]["headers"] = {"X-Signature": "inline-secret"}
        payload["probes"][0]["secret_headers"] = {"X-Signature": "CYBERFUL_WEBHOOK_SIGNATURE"}
        with patch.dict(os.environ, {"CYBERFUL_WEBHOOK_SIGNATURE": "environment-secret"}, clear=False):
            process, evidence = self._run_cli(payload)
        self.assertEqual(process.returncode, 2)
        self.assertIsNone(evidence)
        self.assertEqual(RecordingHandler.requests, [])
    ...
    def test_secret_is_resolved_after_preflight_and_redacted_from_evidence(self) -> None:
        payload = probe_plan(self.origin)
        payload["probes"][0]["secret_headers"] = {"Authorization": "CYBERFUL_PROBE_AUTHORIZATION"}
        with tempfile.TemporaryDirectory() as directory, patch.dict(os.environ, {"CYBERFUL_PROBE_AUTHORIZATION": ""}, clear=False):
            ...
            with self.assertRaisesRegex(self.module.ProbeError, "absent or invalid"):
                self.module._resolved_secrets(probes)
        ...
        self.assertEqual(evidence["probes"][0]["response"]["headers_redactions"], 1)
        self.assertEqual(evidence["probes"][0]["response"]["body_redactions"], 1)
