#!/usr/bin/env python3
# ── Bounded Authorization Campaign Runner ───────────────────────
# Executes a pre-authorized matrix of HTTP requests through curl and preserves
#   bounded raw responses without deciding whether an outcome is vulnerable.
# → cyberful/builtin/skills/test-authorization-boundaries/scripts/manifest.json — execution contract and hard limits.
# → cyberful/builtin/skills/test-authorization-boundaries/assets/authorization-campaign.schema.json — campaign input.
# → cyberful/builtin/skills/test-authorization-boundaries/tests/test_run_authorization_campaign.py — boundary and loopback coverage.
# ─────────────────────────────────────────────────────────────────
...
def _number(value: Any, label: str, *, minimum: float, maximum: float) -> float:
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise CampaignError(f"{label} must be a number")
    normalized = float(value)
    if not minimum <= normalized <= maximum:
        raise CampaignError(f"{label} must be between {minimum} and {maximum}")
    return normalized
...
def _read_json(workspace: Path, value: str) -> tuple[dict[str, Any], bytes, Path]:
    ...
    if not isinstance(payload, dict):
        raise CampaignError("campaign input must be a JSON object")
...
def _parsed_http_url(value: str, label: str) -> SplitResult:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.hostname or parsed.username or parsed.password or parsed.fragment:
        raise CampaignError(f"{label} must be an HTTP(S) URL without credentials or fragment")
    try:
        parsed.port
    except ValueError as error:
    ...
    if hostname == "localhost" or hostname.endswith(".localhost"):
        raise CampaignError(f"{label} must use a literal loopback IP instead of localhost")
    return parsed
...
def _headers(value: Any, label: str, secret_names: frozenset[str]) -> tuple[tuple[str, str], ...]:
    if not isinstance(value, dict) or len(value) > MAX_HEADERS:
        raise CampaignError(f"{label} must be an object with at most {MAX_HEADERS} headers")
    ...
    for raw_name, raw_value in value.items():
        ...
        if not HEADER_NAME.fullmatch(name) or name.lower() in {"host", "content-length"} | SENSITIVE_HEADER_NAMES | secret_names:
            raise CampaignError(f"{label} contains a forbidden header name: {name}")
...
def _secret_headers(value: Any, label: str) -> tuple[tuple[str, str], ...]:
    if not isinstance(value, dict) or len(value) > len(SECRET_ENVIRONMENT):
        raise CampaignError(f"{label} must be an object using declared secret environment variables")
    normalized: list[tuple[str, str]] = []
    for raw_name, raw_environment in value.items():
        ...
        if not HEADER_NAME.fullmatch(name) or environment not in SECRET_ENVIRONMENT:
            raise CampaignError(f"{label} contains an unsupported header or environment variable")
        if name.lower() in {"host", "content-length"}:
            raise CampaignError(f"{label} contains a forbidden secret header name: {name}")
...
def _body(value: Any, label: str) -> bytes:
    if not isinstance(value, str):
        raise CampaignError(f"{label} must be a base64 string")
    try:
        decoded = base64.b64decode(value, validate=True)
    except ValueError as error:
        raise CampaignError(f"{label} must be valid base64") from error
    if len(decoded) > MAX_BODY_BYTES:
        raise CampaignError(f"{label} exceeds the {MAX_BODY_BYTES}-byte limit")
    return decoded


# ── Campaign Limits Precede Host-Owned Routing ──────────────────
# The campaign file carries a reference, exact origins, and bounded request
# limits only as defense in depth; it cannot grant authority or select transport.
# Every case is validated before host-owned route and secret environment values
# are read. Literal loopback IPs bypass every proxy, while non-loopback traffic
# requires the proxy and CA route injected by the Cyberful gateway.
# ─────────────────────────────────────────────────────────────────
def _campaign(payload: dict[str, Any], workspace: Path) -> tuple[str, Authority, tuple[AuthorizationCase, ...]]:
    if set(payload) != {"$schema", "campaign_id", "authority", "cases"}:
        raise CampaignError("campaign must contain exactly $schema, campaign_id, authority, and cases")
    ...
    if not isinstance(authority_value, dict) or set(authority_value) != {
        ...
        raise CampaignError("authority contains missing or unknown fields")
    ...
    if not isinstance(raw_origins, list) or not raw_origins or len(raw_origins) > 16:
        raise CampaignError("authority.allowed_origins must contain between 1 and 16 origins")
    ...
    if any(parsed.path not in {"", "/"} or parsed.query for parsed in parsed_origins):
        raise CampaignError("authority.allowed_origins entries must be origins without path or query")
    ...
    if not isinstance(raw_cases, list) or not raw_cases or len(raw_cases) > max_requests:
        raise CampaignError("cases must be non-empty and must not exceed authority.max_requests")
    required = {"case_id", "actor", "tenant", "resource", "action", "expected_policy", "method", "url", "headers", "secret_headers", "body_base64"}
    secret_headers_by_case: list[tuple[tuple[str, str], ...]] = []
    for index, raw_case in enumerate(raw_cases):
        label = f"cases[{index}]"
        if not isinstance(raw_case, dict) or set(raw_case) != required:
            raise CampaignError(f"{label} contains missing or unknown fields")
        secret_headers_by_case.append(_secret_headers(raw_case["secret_headers"], f"{label}.secret_headers"))
    declared_secret_names = frozenset(name.lower() for headers in secret_headers_by_case for name, _ in headers)
    ...
    for index, (raw_case, secret_headers) in enumerate(zip(raw_cases, secret_headers_by_case, strict=True)):
        ...
        if _origin(parsed_url) not in allowed_origins:
            raise CampaignError(f"{label}.url origin is outside authority.allowed_origins")
        method = _text(raw_case["method"], f"{label}.method", maximum=16).upper()
        if method not in METHODS:
            raise CampaignError(f"{label}.method is unsupported")
        expected_policy = _text(raw_case["expected_policy"], f"{label}.expected_policy", maximum=16)
        if expected_policy not in {"allow", "deny"}:
            raise CampaignError(f"{label}.expected_policy must be allow or deny")
        cases.append(AuthorizationCase(
            case_id=_text(raw_case["case_id"], f"{label}.case_id"),
            actor=_text(raw_case["actor"], f"{label}.actor"),
            tenant=_text(raw_case["tenant"], f"{label}.tenant"),
            resource=_text(raw_case["resource"], f"{label}.resource"),
            action=_text(raw_case["action"], f"{label}.action"),
            expected_policy=expected_policy,
            method=method,
            url=url,
            headers=_headers(raw_case["headers"], f"{label}.headers", declared_secret_names),
            secret_headers=secret_headers,
            body=_body(raw_case["body_base64"], f"{label}.body_base64"),
        ))
    case_ids = [case.case_id for case in cases]
    if len(case_ids) != len(set(case_ids)):
        raise CampaignError("case_id values must be unique")
    return campaign_id, Authority(authorization_reference, allowed_origins, max_requests, requests_per_second, request_timeout_seconds), tuple(cases)


def _runtime_route(cases: tuple[AuthorizationCase, ...]) -> RuntimeRoute:
    ...
    for scheme in schemes:
        ...
        if not raw_proxy:
            raise CampaignError(f"non-loopback {scheme} targets require the host-provided {environment_name} route")
        proxy = _parsed_http_url(raw_proxy, environment_name)
        if proxy.path not in {"", "/"} or proxy.query or proxy.port is None:
            raise CampaignError(f"{environment_name} must be an explicit proxy origin")
        proxies[scheme] = raw_proxy
    raw_ca = os.environ.get("CURL_CA_BUNDLE") or os.environ.get("SSL_CERT_FILE")
    ca_bundle: Path | None = None
    if schemes:
        if not raw_ca:
            raise CampaignError("non-loopback targets require host-provided CURL_CA_BUNDLE or SSL_CERT_FILE")
        ...
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_size > 1_048_576:
            raise CampaignError("runtime CA bundle must be a regular file no larger than 1048576 bytes")
...
def _terminate_process_group(process: subprocess.Popen[bytes]) -> None:
    ...
    try:
        ...
        raise CampaignError("curl process group could not be reaped") from error
...
def _run_process(
    ...
    *,
    ...
    try:
        ...
        raise CampaignError(f"could not start curl: {error}") from error
    if process.stdout is None or process.stderr is None:
        _terminate_process_group(process)
        raise CampaignError("curl output pipes were not created")
...
def _resolved_secrets(cases: tuple[AuthorizationCase, ...]) -> dict[str, str]:
    ...
    for case in cases:
        for _, environment in case.secret_headers:
            ...
            if not secret or len(secret.encode("utf-8")) > MAX_SECRET_BYTES or any(ord(character) < 32 or ord(character) == 127 for character in secret):
                raise CampaignError(f"required secret environment variable is absent or invalid: {environment}")
            resolved[environment] = secret
    return resolved


def _execute_case(case: AuthorizationCase, authority: Authority, route: RuntimeRoute, secret_values: dict[str, str], global_deadline: float) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="cyberful-auth-probe-") as directory:
        ...
        request_body_path.write_bytes(case.body)
        os.chmod(request_body_path, 0o600)
        config_lines = [f'header = "{_curl_quote(name)}: {_curl_quote(value)}"' for name, value in case.headers]
        resolved_headers = tuple((name, environment, secret_values[environment]) for name, environment in case.secret_headers)
        ...
        argv = [
            ...
            case.url,
            "--connect-timeout",
            str(min(10, authority.request_timeout_seconds)),
            "--max-time",
            str(authority.request_timeout_seconds),
            "--max-filesize",
        ...
        if _is_loopback(parsed_url):
            ...
            if proxy_url is None:
                raise CampaignError("non-loopback request lost its host-provided proxy route")
        ...
        if remaining <= 0:
            raise CampaignError("campaign exceeded its global deadline")
        ...
        secret_values_for_case = tuple(secret for _, _, secret in resolved_headers)
        raw_headers, header_redactions, redacted_headers_truncated = _redact(raw_headers, secret_values_for_case, MAX_HEADER_BYTES)
        raw_body, body_redactions, redacted_body_truncated = _redact(raw_body, secret_values_for_case, MAX_CAPTURE_BYTES)
        stderr, stderr_redactions, _ = _redact(process.stderr, secret_values_for_case, MAX_STDERR_BYTES)
...
def run_campaign(payload: dict[str, Any], workspace: Path, source_sha256: str) -> dict[str, Any]:
    ...
    secret_values = _resolved_secrets(cases)
    ...
    for case in cases:
        ...
        if delay > 0:
            if time.monotonic() + delay >= deadline:
                raise CampaignError("campaign deadline expired before the next request")
            time.sleep(delay)
        results.append(_execute_case(case, authority, route, secret_values, deadline))
        if time.monotonic() >= deadline:
            raise CampaignError("campaign exceeded its global deadline")
        next_request_at = max(next_request_at + interval, time.monotonic())
    return {
        "format": "cyberful.authorization-campaign-evidence.v1",
        ...
        "cases": results,
        "interpretation": "Raw bounded transport evidence only; compare current policy, controls, reproducibility, security effect, and impact before reaching a vulnerability conclusion.",
    }
...
def _write_report(workspace: Path, value: str, report: dict[str, Any], source: Path) -> None:
    ...
    if destination == source:
        raise CampaignError("output must not replace the source campaign")
    ...
    if len(rendered) > MAX_OUTPUT_BYTES:
        raise CampaignError(f"rendered evidence exceeds the {MAX_OUTPUT_BYTES}-byte limit")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run a bounded authorized HTTP authorization campaign.")
    ...
    try:
        ...
        report = run_campaign(payload, workspace, hashlib.sha256(raw).hexdigest())
