#!/usr/bin/env python3
# ── Mission-Routed AI Tool-Authorization Probe ──────────────────
# Sends matched allowed/denied actions through Cyberful's runtime route and
# retains bounded, secret-redacted evidence under campaign constraints.
# → cyberful/builtin/skills/test-ai-tool-authorization/assets/tool-authorization-probe.schema.json — input contract.
# → cyberful/builtin/skills/test-ai-tool-authorization/tests/test_run_tool_authorization_probe.py — forward tests.
# ─────────────────────────────────────────────────────────────────
...
AUTHORIZATION_ENV: Final = "CYBERFUL_AI_TOOL_AUTHORIZATION"
...
def _action(value: Any, label: str) -> dict[str, Any]:
    if not isinstance(value, dict) or not value:
        raise ProbeError(f"{label} must be a non-empty object")
    try:
        encoded = json.dumps(value, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
    except (TypeError, ValueError) as error:
        raise ProbeError(f"{label} must be JSON serializable") from error
    if len(encoded) > MAX_ACTION_BYTES:
        raise ProbeError(f"{label} exceeds its byte boundary")
    return value
...
def _validate(payload: dict[str, Any]) -> tuple[Constraints, list[dict[str, Any]]]:
    ...
    required = {"id", "endpoint", "actor_id", "tenant_id", "effect", "allowed_action", "denied_action"}
    for index, case in enumerate(cases):
        ...
        normalized.append({
            ...
            "effect": effect,
            "allowed_action": _action(case["allowed_action"], f"cases[{index}].allowed_action"),
            "denied_action": _action(case["denied_action"], f"cases[{index}].denied_action"),
        })
...
def _send(endpoint: str, body: dict[str, Any], deadline: float, expiry: datetime, context: ssl.SSLContext, proxy: str | None, secret: str | None) -> tuple[dict[str, Any], int]:
    if datetime.now(timezone.utc) >= expiry:
        raise ProbeError("campaign constraints expired during execution")
    started = time.monotonic()
    remaining = deadline - started
    if remaining <= 0:
        raise ProbeError("tool-authorization probe exceeded its global deadline")
...
def run_probe(payload: dict[str, Any], digest: str, workspace: Path, *, deadline_seconds: float = PROBE_TIMEOUT_SECONDS, evidence_limit_bytes: int = MAX_OUTPUT_BYTES) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        "format": "cyberful.ai-tool-authorization-probe.raw.v1",
        "input_sha256": digest,
    ...
    for case in cases:
        ...
        case_record = {"case_id": case["id"], "endpoint": case["endpoint"], "actor_id": case["actor_id"], "tenant_id": case["tenant_id"], "effect": case["effect"], "observations": observations}
        _bounded_candidate(report, case_record, evidence_limit_bytes)
        for expectation, key in (("allowed", "allowed_action"), ("denied", "denied_action")):
            delay = next_request_at - time.monotonic()
            if delay > 0:
                if time.monotonic() + delay >= deadline:
                    raise ProbeError("tool-authorization probe exceeded its global deadline")
                time.sleep(delay)
            observation, raw_evidence_bytes = _send(case["endpoint"], {"action": case[key], "expectation": expectation, "actor_id": case["actor_id"], "tenant_id": case["tenant_id"], "effect": case["effect"]}, deadline, constraints.expires_at, context, case["proxy"], secret)
            staged = {**case_record, "observations": [*observations, {"expectation": expectation, **observation}]}
            _bounded_candidate(report, staged, evidence_limit_bytes, source_evidence_bytes + raw_evidence_bytes)
            observations.append({"expectation": expectation, **observation})
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run a mission-routed matched AI tool-authorization probe.")
    ...
    try:
        ...
        print(f"tool-authorization probe error: {error}", file=sys.stderr)
        return 2
