...
- Read all prior Code Audit artifacts. Load `audit-application-code`, `operate-code-graph`, and applicable domain skills such as `audit-native-memory-safety`, `trace-injection-dataflows`, `test-data-protection-crypto`, and `audit-software-supply-chain`.
